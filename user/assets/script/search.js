...
document.getElementById('searchBox2').addEventListener('input', function () {
    const searchValueT = this.value.toLowerCase();
    const rowsT = document.querySelectorAll('#reservationsTable tbody tr');
    rowsT.forEach(row => {
        const columns = row.querySelectorAll('td');
        let matchFound = false;
        columns.forEach(column => {
            if (column.textContent.toLowerCase().includes(searchValueT)) {
                matchFound = true;
            }
        });
        if (matchFound) {
